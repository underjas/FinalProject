...
function createMap() {
    ...
    L.tileLayer('https://api.mapbox.com/styles/v1/underjas/cm6zlihch00ef01sle41zhier/tiles/{z}/{x}/{y}?access_token={accessToken}', {
     ...
     // Calculate the bounds (1 degree lat/lng from the center)
     ...
     // Prevent moving outside bounds with inertia
...
// Configure popup for the garage layer
garageLayer.bindPopup(function() {
    let popupContent = '<h4>OSU Parking Garage</h4>';
    popupContent += '<p><a href="#" class="view-details-link" onclick="window.open(\'https://underjas.github.io/ParkingGarage\', \'popup\', \'width=600,height=400,toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes\'); return false;">View Details</a></p>';
    return popupContent;
});
            ...
            input.addEventListener('change', function(event) {
                const selectedGroup = event.target.value;
                    if (displayGroups[selectedGroup]) {
                        ...
                        filterZonesByGroup([selectedGroup]);
                    }
            });
        });
...
function filterZonesByGroup(groups) {
    if (typeof geojson !== 'undefined') {
        geojson.eachLayer(function(layer) {
            if (groups.includes(layer.feature.properties.ZoneGroup)) {
                layer.addTo(map);
            } else {
        ...
        console.error('GeoJSON layer is not defined. Ensure ParkingZones.geojson is loaded.');
    }
}
