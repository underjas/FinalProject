...
function filterZonesByGroup(selectedGroup) {
    ...
    geojson.eachLayer(function(layer) {
        ...
        if (selectedGroup === 'All' || 
            (selectedGroup === 'A1' && /^(A1|B1|B2|B3|C)$/.test(zoneGroup)) ||
            zoneGroup === selectedGroup) {
                layer.addTo(map);
        } else {
...
const parkingZoneFilter = L.control({ position: 'topright' });

parkingZoneFilter.onAdd = function (map) {
    const div = L.DomUtil.create('div', 'filter-control');
    div.innerHTML = `
        <h4>Filter Parking Zones</h4>
        <label><input type="radio" name="zone" value="all" checked> All</label><br>
        <label><input type="radio" name="zone" value="a1"> A1, B1, B2, B3, C</label><br>
        <label><input type="radio" name="zone" value="a2"> A2, B1, B2, B3, C</label><br>
        <label><input type="radio" name="zone" value="a3"> A3, B1, B2, B3, C</label><br>
        <label><input type="radio" name="zone" value="b1"> B1, C</label><br>
        <label><input type="radio" name="zone" value="b2"> B2, C</label><br>
        <label><input type="radio" name="zone" value="b3"> B3, C</label><br>
    `;
    L.DomEvent.disableClickPropagation(div);
    return div;
};

parkingZoneFilter.addTo(map);
...
document.querySelectorAll('input[name="zone"]').forEach(input => {
    input.addEventListener('change', function() {
        ...
        geojson.eachLayer(layer => {
            ...
            } else if (selectedZone === 'a1' && ['A1', 'B1', 'B2', 'B3', 'C'].includes(zone)) {
                layer.setStyle({ fillOpacity: 0.7 });
            } else if (selectedZone === 'a2' && ['A2', 'B1', 'B2', 'B3', 'C'].includes(zone)) {
                layer.setStyle({ fillOpacity: 0.7 });
            } else if (selectedZone === 'a3' && ['A3', 'B1', 'B2', 'B3', 'C'].includes(zone)) {
                layer.setStyle({ fillOpacity: 0.7 });
            } else if (selectedZone === 'b1' && ['B1', 'C'].includes(zone)) {
                layer.setStyle({ fillOpacity: 0.7 });
            } else if (selectedZone === 'b2' && ['B2', 'C'].includes(zone)) {
                layer.setStyle({ fillOpacity: 0.7 });
            } else if (selectedZone === 'b3' && ['B3', 'C'].includes(zone)) {
                layer.setStyle({ fillOpacity: 0.7 });
            } else {
